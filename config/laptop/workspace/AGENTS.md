...
- Owner wants an EXISTING bot/pipeline to DO WORK on their files ("run", "process this", "use bot 013 on"): `python tools/factory.py queue run <bot_id> --task "<what to do>" --in <folder>` or `python tools/factory.py queue run --plan <plan_job_id> --in <folder>`; outputs land in `C:\AI\Factory\run\runs\`. Ask for the folder path if none was given.
