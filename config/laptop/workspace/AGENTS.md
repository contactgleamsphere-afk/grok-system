...
- Owner asks to CHANGE THE FACTORY ITSELF ("improve the factory so that…", "change how bots are tested", "fix the factory's…"): `python tools/factory.py queue selfpatch --request "<owner's request verbatim>"`. Tell the owner the factory will draft the change as a GitHub pull request for them to review — it never changes itself directly. Never run `git`, never edit factory files yourself.
