...
$c=Get-Content $p -Raw|ConvertFrom-Json; $s=Get-Content C:\AI\Factory\config.keyed-lanes.staged.json -Raw|ConvertFrom-Json
...
$chain=@(); foreach($x in @('groq-llama70b','groq-gptoss120b','or-free-auto','gemini-flash','ovh-gptoss20b','ovh-mistral24b','local4b')){ if($c.modelPresets.PSObject.Properties[$x]){ $chain+=$x } }
