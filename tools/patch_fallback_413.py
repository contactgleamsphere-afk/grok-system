...
new='''        # AI-Factory patch (2026-09-19): Groq reports per-minute token caps as
        ...
        if ("tool_use_failed" in code or "tool call validation failed" in text[:400]
                or "failed_generation" in text[:600] or "parsing failed. the model generated output" in text[:400]):
            return True
        ...
        if any(
            ...
            return False
        if status in {401, 403}:'''
if "failed_generation" in s: print("already patched")
elif "tool call validation failed" in s:
    s=s.replace('''        if "tool_use_failed" in code or "tool call validation failed" in text[:400]:
            return True''','''        if ("tool_use_failed" in code or "tool call validation failed" in text[:400]
                or "failed_generation" in text[:600] or "parsing failed. the model generated output" in text[:400]):
            return True'''); p.write_text(s,encoding="utf-8"); print("extended: failed_generation fallbackable")
